...
const ManageClasses = () => {
  const {
    ...
    const res = await fetch("https://top-musicy-server.vercel.app/allclasses");
    return res.json();
  });
  ...
  const handlePending = (status) => {
    fetch(`https://top-musicy-server.vercel.app/class/${status}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeny = (status) => {
    fetch(`https://top-musicy-server.vercel.app/deny/${status}`, {
      method: "PATCH",
    })
      .then((res) => res.json())
      .then((data) => {
            ...
            <tbody>
              ...
              {classes?.map((classItem) => (
                <tr key={classItem.id}>
                  ...
                  <td className="w-1/2">
                    {classItem.className?.slice(1, 30)}.....
                  </td>
                  ...
                  <td className="w-full flex flex-col">
                    ...
                    {classItem.status === "denied" ? (
                      <button className="btn btn-sm btn-disabled mb-3 w-full font-bold text-red-500">
                        Denied
                      </button>
                    ) : (
                      <button
